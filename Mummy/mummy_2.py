...
way = pygame.image.load("./image/way.PNG")
way = pygame.transform.scale(way, (100, 100))
way_x, way_y = 500, 200
way_width, way_height = 100, 100

win = pygame.image.load("./image/end.png")
win = pygame.transform.scale(win, (600, 600))
win_game = False

try_again_img = pygame.image.load("./image/tryagain_red.png")
try_again_img = pygame.transform.scale(try_again_img, (300, 100))
try_again = False

wallX1, wallY1, wallW1, wallH1 = 400, 200, 10, 100
wallX2, wallY2, wallW2, wallH2 = 200, 400, 100, 10
wallX3, wallY3, wallW3, wallH3 = 300, 300, 10, 100
...
class Mummy:
    ...
    def run(self, keyPlayer, playerX, playerY):
        ...
        if self.x != self.keyRun['X']:
            ...
            keyWall_3 = getKey(wallX3, wallY3)  # 21
            
            # wall dọc đỏ
            if keyMummy == keyWall_3:               
                if playerY == wallY3:
                    Player.mummyGo = 0
                else:
...
while True:
    ...
    if not try_again and not win_game:
        ...
        DISPLAYSURF.blit(way, (way_x, way_y))
        ...
        if mummy.check_collision(player):
            try_again = True  # Dừng game khi va chạm thực sự
            DISPLAYSURF.blit(try_again_img, (150, 250))  # Hiển thị "Try Again"

        if player.x == way_x and player.y == way_y:
            win_game = True
